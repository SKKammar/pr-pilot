...
const DEMO_DETAIL_MAP: Record<string, { review: ReviewRecord; comments: ReviewCommentRecord[] }> = {
  ...
  "rev-demo-02": {
    ...
    comments: [
      {
        id: "c3",
        filename: "packages/core/src/stateMachine.ts",
        line_pos: 124,
        severity: "error",
        message: "Stale closure in transition batch: Interrupted transition phase does not reset pending action queue, causing dropped actions.",
        suggestion: "pendingActions.clear();\nresetTransitionState(action.id);",
      },
      {
        id: "c4",
        filename: "packages/core/src/useFlowSync.ts",
        line_pos: 67,
        severity: "suggestion",
        message: "Inline allocation inside hot loop can trigger unnecessary garbage collection cycles during high-frequency telemetry streaming.",
        suggestion: "const memoizedSyncPayload = useMemo(() => buildPayload(data), [data]);",
      },
    ],
  },
  "rev-demo-03": {
    ...
    comments: [
      {
        id: "c5",
        filename: "scanner/engine/pattern_matcher.py",
        line_pos: 53,
        severity: "warning",
        message: "Unbounded dictionary cache may exhaust worker memory during sustained scanning of high-entropy codebases.",
        suggestion: "from collections import OrderedDict\npattern_cache = OrderedDict()  # Apply maxsize=2048 eviction",
      },
      {
        id: "c6",
        filename: "scanner/rules/entropy.py",
        line_pos: 88,
        severity: "suggestion",
        message: "Redundant Shannon entropy calculation can be short-circuited for strings shorter than 16 characters.",
        suggestion: "if len(token) < 16:\n    return 0.0",
      },
    ],
  },
  "rev-demo-04": {
    ...
    comments: [
      {
        id: "c7",
        filename: "middleware/security_headers.go",
        line_pos: 31,
        severity: "suggestion",
        message: "Include HSTS preload directive to meet modern enterprise security standards.",
        suggestion: "w.Header().Set(\"Strict-Transport-Security\", \"max-age=63072000; includeSubDomains; preload\")",
      },
    ],
  },
  "rev-demo-05": {
